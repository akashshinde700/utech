...
const { createUserSchema, updateUserSchema, setUserPermissionsSchema } = require('../validators/user.schema');
...
router.get('/:id/permissions', requirePermission('user.update'), ah(ctrl.listPermissions));
router.put('/:id/permissions', requirePermission('user.update'), validate(setUserPermissionsSchema), ah(ctrl.setPermissions));
