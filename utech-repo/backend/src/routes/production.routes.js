...
} = require('../validators/production.schema');

router.use(requireAuth);

// Shift management
router.get('/shifts', requirePermission('jobcard.read'), ah(ctrl.listShifts));
router.post('/shifts', requirePermission('jobcard.create'), validate(shiftSchema), ah(ctrl.createShift));
router.put('/shifts/:id', requirePermission('jobcard.update'), validate(updateShiftSchema), ah(ctrl.updateShift));
...
router.post('/batches', requirePermission('jobcard.create'), validate(createBatchSchema), ah(ctrl.createBatch));
router.put('/batches/:id', requirePermission('jobcard.update'), validate(updateBatchSchema), ah(ctrl.updateBatch));
...
router.post('/batches/:id/materials', requirePermission('jobcard.update'), validate(updateMaterialConsumptionSchema), ah(ctrl.updateMaterialConsumption));
